...
class NormalDataset(BaseDataset):
    ...
    def get_sample(self, idx) -> np.ndarray:
        return self.transforms(Image.open(self.files[idx])).numpy()
...
def get_datasets_cxr(config: Namespace,
...
def get_dataloaders_cxr(config: Namespace,
    ...
    """
    dset1, dset2 = get_datasets_cxr(config, train)
