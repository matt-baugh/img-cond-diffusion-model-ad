...
def get_camcan_files(config) -> List[str]:
    ...
    """
    path = os.path.join(config.datasets_dir, 'MRI/CamCAN')
...
def get_brats_files(config) -> Tuple[List[str], List[str]]:
    ...
    """
    files = sorted(glob(os.path.join(config.datasets_dir, 'MRI/BraTS/MICCAI_BraTS2020_TrainingData/*',
...
def get_atlas_files(config) -> List[str]:
    ...
    if config.get_images:
        ...
        assert False, "Legacy code from UPD study development??"
    else:
        path = os.path.join(config.datasets_dir, 'MRI/ATLAS/ATLAS_2/Training')
