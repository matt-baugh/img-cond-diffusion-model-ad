...
class PIITask:
    ...
    def __init__(self, is_mri: bool):
        self.is_mri = is_mri

    def apply(self, img: np.ndarray, dset: BaseDataset) -> tuple[np.ndarray, np.ndarray]:
        img2 = dset.get_sample(np.random.randint(0, len(dset)))
        res = pii(img, img2, is_mri=self.is_mri)
        return res
...
config.aug_fn = PIITask(is_mri=config.modality == 'MRI')
